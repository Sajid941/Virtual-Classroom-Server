...
router.get("/classid", async (req, res) => {
    ...
    try {
        ...
        res.status(classe ? 200 : 404).json(
            classe || { message: "Class not found" }
        );
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
router.patch("/:classId", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        res.status(updatedClass ? 200 : 404).json(
            updatedClass
...
router.patch("/:classId/students", async (req, res) => {
    ...
    try {
        ...
        res.status(classData ? 200 : 404).json(
            classData || { message: "Class not found" }
        );
    } catch (error) {
...
router.patch("/:classId/quiz", async (req, res) => {
    ...
    try {
        const classData = await Class.findOneAndUpdate(
            ...
            {
                $addToSet: {
                    quizzes: { $each: Array.isArray(quiz) ? quiz : [quiz] },
                },
            },
...
router.patch("/:classId/quizsubmission", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            message: "Failed to update quiz submissions",
            error,
        });
    }
});
    ...
    fs.access(filePath, fs.constants.F_OK, (err) => {
        ...
        res.download(filePath, filename, (err) => {
            if (err) {
                return res
                    .status(500)
                    .json({ message: "Error downloading file", err });
            }
        });
    });
});
...
router.get("/user-submissions", async (req, res) => {
    try {
        ...
        if (assignmentName && assignmentName !== "all") {
            query["assignments.title"] = {
                $regex: assignmentName,
                $options: "i",
            };
        }
        if (search) {
